...
fillSmallest smallest =
  map
    ( concatMap
        (\x -> if length x == length (item smallest) then item smallest else x)
        . chunks (length $ item smallest)
    )
...
chestValue treasures chest =
  let treasureValue t =
        ( fst
            . flip expect "a treasure has to be non-empty!"
            . uncons
            $ item t,
          (fromIntegral $ value t :: Float) / fromIntegral (length $ item t)
        )
  ...
  let maxValue a b = if chestValue treasures a > chestValue treasures b then a else b
      ...
      check (a : as, b : bs) =
        a
          : optimizeTreasure
            treasures
            (as ++ maxValue (a ++ item treasure) b : bs)
            treasure
      check (_, _) = chests
   in check split
...
fillTreasures treasures@(smallest : other) chestSize =
  foldl
    (optimizeTreasure treasures)
    (fillSmallest smallest $ emptyChests chestSize)
    other

printChests :: [String] -> IO ()
